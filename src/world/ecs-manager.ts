...
import { Transform, renderedProxyDef, renderedUiProxyDef } from '../built-in-ecs-components';
...
import { UiBuildTreeNode, UiNodeType, UiTreeNode } from '../ui/ui-tree-node';
...
export class EcsManager {
  ...
  constructor(private canvas: fabric.StaticCanvas, private uiCanvas: fabric.Canvas) {}
  ...
  getResource<T>(resourceIdentifier: ResourceMethods<T>) {
    return this.resourcesCointainer.getResourceFromContainer(resourceIdentifier);
  }
  ...
  setState<T extends number>(stateDefintion: StateDefinition<T>, value: T) {
    let currStateDetail = this.statesContainer.get(stateDefintion.getIdentifier());
    if (!currStateDetail) {
  ...
  replaceUiElement(entityId: number, elementTargetId: string, replaceWith: UiBuildTreeNode) {
    let entityIndex = this.entities.findIndex((x) => x.id === entityId);
    let entity = this.entities[entityIndex];

    let renderedUiComponent = entity.components[renderedUiProxyDef.getIdentifier()];
    if (renderedUiComponent !== undefined) {
      let ui = renderedUiComponent as EntityUiObjectProxy;
      let { toAdd, toRemove } = ui.replaceElementInUi(elementTargetId, replaceWith);
      this.removeFromUiCanvas(toRemove);
      this.addToUiCanvas(toAdd);
    }
  }

  changeTextUiElement(entityId: number, elementTargetId: string, replaceWithText: string) {
    let entityIndex = this.entities.findIndex((x) => x.id === entityId);
    let entity = this.entities[entityIndex];
  ...
  initEntity<T extends Record<string, ComponentMethods<any>>>(
    ...
    for (const key in entityInitDefinitions) {
      ...
      if (comp.getIdentifier() === renderedProxyDef.getIdentifier()) {
        this.addToCanvas((entityInit[key] as EntityObjectProxy).getRenderedObject());
      }
      if (comp.getIdentifier() === renderedUiProxyDef.getIdentifier()) {
        this.addToUiCanvas((entityInit[key] as EntityUiObjectProxy).getRenderedObject());
      }
      comp.addComponentToEntity(entity, entityInit[key]);
      this.setLifecycleState({ entityId: entity.id, componentId: comp.getIdentifier() }, 'created');
    }
  ...
  removeEntity(entityId: number) {
    ...
    if (renderedComponent !== undefined) {
      this.removeFromCanvas((renderedComponent as EntityObjectProxy).getRenderedObject());
    }
    if (renderedUiComponent !== undefined) {
      this.removeFromUiCanvas((renderedUiComponent as EntityUiObjectProxy).getRenderedObject());
    }
    this.entities.splice(entityIndex, 1);
  }

  addComponent<T>(entityId: number, component: T, componentDefintion: ComponentMethods<T>) {
    let entity = this.entities[entityId];
    entity.components[componentDefintion.getIdentifier()] = component;
    this.setLifecycleState({ entityId, componentId: componentDefintion.getIdentifier() }, 'created');
    if (componentDefintion.getIdentifier() === renderedProxyDef.getIdentifier()) {
      this.addToCanvas((component as EntityObjectProxy).getRenderedObject());
    } else if (componentDefintion.getIdentifier() === renderedUiProxyDef.getIdentifier()) {
      this.addToUiCanvas((component as EntityUiObjectProxy).getRenderedObject());
    }
  }

  removeComponent<T>(entityId: number, componentDefintion: ComponentMethods<T>) {
  ...
  setLifecycleState(key: { entityId: number; componentId: string }, value: LifecycleStatus) {
    this.lifecycleStatusMap.set(JSON.stringify(key), value);
  }

  isInLifecycleStats(entityId: number, componentId: string, status: LifecycleStatus) {
    return this.lifecycleStatusMap.get(JSON.stringify({ entityId, componentId })) === status;
  }
